...
```
packwiz-util [--index=index.toml] [--cf-api-key='CF_API_KEY'] [--cf-detect] [--cf-url] [--mr-detect] [--mr-merge]
```

- `--index` path to the packwiz pack's `index.toml` file, defaults to `index.toml` in the working directory
- `--cf-api-key` required for all `--cf` operations, can also be provided via `CF_API_KEY` environment variable or `.env` file
